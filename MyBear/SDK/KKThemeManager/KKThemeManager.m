...
@synthesize currentThemeName,themeInformation;

- (void)dealloc{
    [currentThemeName release];
    [themeInformation release];
    [super dealloc];
}
...
- (id)init {
    self = [super init];
    if (self) {
        themeInformation = [[NSMutableDictionary alloc]init];
        [self initTheme];
    }
    return self;
}
...
- (void)initTheme{
    ...
    currentThemeName = [app_Name copy];
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:themePath]) {
        ...
        [currentThemeName release];
        currentThemeName = nil;
        currentThemeName = [DefaultThemeBundleName copy];
    }
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:themePath]) {
        ...
        if (themeInfo && [themeInfo isKindOfClass:[NSDictionary class]]) {
            [themeInformation removeAllObjects];
            [themeInformation setValuesForKeysWithDictionary:themeInfo];
            return;
        }
        else{
            [themeInformation removeAllObjects];
            #ifdef DEBUG
            NSLog(@"warning:【主题加载】失败！%@.bundle 存在,但是里面的theme.plist不是Dictionary对象",app_Name);
            #endif
            return;
        }
    }
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:themePath]) {
        ...
        if (themeInfo && [themeInfo isKindOfClass:[NSDictionary class]]) {
            [themeInformation removeAllObjects];
            [themeInformation setValuesForKeysWithDictionary:themeInfo];
            currentThemeName = [app_ShowName copy];
            return;
        }
        else{
            [themeInformation removeAllObjects];
            #ifdef DEBUG
            NSLog(@"warning:【主题加载】失败！%@.bundle 存在,但是里面的theme.plist不是Dictionary对象",app_ShowName);
            #endif
            return;
        }
    }
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:themePath]) {
        ...
        if (themeInfo && [themeInfo isKindOfClass:[NSDictionary class]]) {
            [themeInformation removeAllObjects];
            [themeInformation setValuesForKeysWithDictionary:themeInfo];
            currentThemeName = [defaultThemeName copy];
            return;
        }
        else{
            [themeInformation removeAllObjects];
            #ifdef DEBUG
            NSLog(@"warning:【主题加载】失败！%@.bundle 存在,但是里面的theme.plist不是Dictionary对象",defaultThemeName);
            #endif
            return;
        }
    }
...
#ifdef DEBUG
    NSLog(@"warning:主题加载失败！%@.bundle/theme.plist、%@.bundle/theme.plist 、%@.bundle/theme.plist 没有一个存在",app_Name,app_ShowName,defaultThemeName);
#endif
...
- (void)reloadThemeWithThemeBundleName:(NSString*)themeBundleName{
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:themePath]) {
        ...
        if (themeInfo && [themeInfo isKindOfClass:[NSDictionary class]]) {
            ...
            [currentThemeName release];
            currentThemeName = nil;
            currentThemeName = [themeBundleName copy];
            ...
            #ifdef DEBUG
            NSLog(@"warning:主题加载失败！%@.bundle 不存在",themeBundleName);
            #endif
            return;
        }
    }
...
#ifdef DEBUG
        NSLog(@"warning:【主题加载】图片：%@，没有在theme.plist里面配置信息",key);
#endif
        return nil;
    }
...
#ifdef DEBUG
            NSLog(@"warning:【主题加载】图片：%@找到了路径：%@，但是并非图片资源",key,imagePath);
#endif
            return nil;
        }
    }
...
#ifdef DEBUG
        NSLog(@"warning:【主题加载】图片：%@找不到路径%@",key,imagePath);
#endif
        return nil;
    }
}
...
#ifdef DEBUG
        NSLog(@"warning:【主题加载】图片：%@找不到路径%@",key,imagePath1x);
#endif
        return nil;
    }
}
...
+ (UIColor *)colorWithKey:(NSString *)key {
    ...
    if ([colorString hasPrefix:@"#"]) {
        ...
        if ([colorList count] > 3) {
            ...
            #ifdef DEBUG
            NSLog(@"warning:【主题加载】颜色：颜色值格式错误%@，格式应该是#FFFFFF或者R,G,B,A 例如134,135,136,1.0",key);
            #endif
            return nil;
        }
    }
